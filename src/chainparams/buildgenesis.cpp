...
int mc_GenerateConfFiles(const char *network_name)
{
    ...
    fileHan=mc_OpenFile(NULL,"treasurelandcoin",".conf","r",MC_FOM_RELATIVE_TO_DATADIR | MC_FOM_CREATE_DIR);
    if(fileHan == NULL)
    {
        fileHan=mc_OpenFile(NULL,"treasurelandcoin",".conf","w",MC_FOM_RELATIVE_TO_DATADIR);
    }
    if(fileHan)
    {
    ...
    fileHan=mc_OpenFile(network_name,"treasurelandcoin",".conf","r",MC_FOM_RELATIVE_TO_DATADIR | MC_FOM_CREATE_DIR);
    if(fileHan == NULL)
    {
        fileHan=mc_OpenFile(network_name,"treasurelandcoin",".conf","w",MC_FOM_RELATIVE_TO_DATADIR);
        if(fileHan)
        {
            fprintf(fileHan,"rpcuser=treasurelandcoinrpc\n");
...
int mc_TreasurelandcoinParams::Build(const unsigned char* pubkey, int pubkey_size) 
{
    ...
    if(mc_gState->m_Features->Streams())
    {
        ...
        if(IsProtocolTreasurelandcoin() == 0)
        {
            root_stream_name_size=0;
        }    
    }
