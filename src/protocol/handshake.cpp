...
bool TreasurelandcoinNode_DisconnectRemote(CNode *pnode)
{
    ...
    {
        return !TreasurelandcoinNode_CanConnect(pnode);
    }
...
bool TreasurelandcoinNode_AcceptData(CNode *pnode)
{
    ...
    return !TreasurelandcoinNode_DisconnectRemote(pnode);
}
...
bool VerifyTreasurelandcoinVerackHash(string sParameterSetHash,uint64_t nNonce)
{
...
{   
    ...
    {
        if(!VerifyTreasurelandcoinVerackHash(sParameterSetHash,nNonce))
        {
            if(!fIsVerackack)
            {
...
//                return false;
    ...
    if(mc_gState->m_NetworkParams->GetInt64Param("anyonecanconnect") == 0)
    {
        ...
        if(fIsVerackack)
        {
            ...
            if(!TreasurelandcoinNode_CanConnect(pfrom))
            {
