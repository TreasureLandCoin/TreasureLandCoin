...
/* MCHN START */    
    ...
    strUsage += "  -saveclilog=<n>          " + _("If <n>=0 treasurelandcoin-cli history is not saved, default 1") + "\n";
/*    
...
//    ParseParameters(argc, argv);
      ...
      {
        fprintf(stdout,"\nTreasureLandCoin Core RPC client %s\n\n",mc_gState->GetFullVersion());
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  treasurelandcoin-cli [options] <command> [params]  " + _("Send command to TreasureLandCoin Core") + "\n" +
                  "  treasurelandcoin-cli [options] help                " + _("List commands") + "\n" +
                  "  treasurelandcoin-cli [options] help <command>      " + _("Get help for a command") + "\n";
...
*/
    ...
    SelectTreasureLandCoinBaseParams(mc_gState->m_Params->NetworkName(),RPCPort);
...
{
    char *chainName = "treasurelandcoin";
    char *newargs[argc+1];
    newargs[1] = chainName;
    ...
    SetupEnvironment();
    try {
        int ret = AppInitRPC(argc+1, newargs);
    ...
    if(mc_gState->m_Params->m_NumArguments == 1)                                // Interactive mode
    {
        ...
        while(!exitnow && ((command=term->GetLine()) != NULL))
        {
            ...
            else
            {
                ...
                offset=0;
                argc_p=0;
                strcpy(dest+offset,"treasurelandcoin-cli");
    ...
    try {
        mc_SaveCliCommandToLog(path_cli_log.string().c_str(), argc+1, newargs);
        ret = CommandLineRPC(argc+1, newargs);
    }
    catch (std::exception& e) {
        PrintExceptionContinue(&e, "CommandLineRPC()");
    } catch (...) {
