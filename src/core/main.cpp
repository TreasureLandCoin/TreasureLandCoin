...
bool AcceptTreasureLandCoinTransaction(const CTransaction& tx, 
...
bool ProcessTreasurelandcoinVerack(CNode* pfrom, CDataStream& vRecv,bool fIsVerackack,bool *disconnect_flag);
bool PushTreasureLandCoinVerack(CNode* pfrom, bool fIsVerackack);
bool TreasurelandcoinNode_CanConnect(CNode *pnode);
bool TreasurelandcoinNode_DisconnectRemote(CNode *pnode);
bool TreasurelandcoinNode_DisconnectLocal(CNode *pnode);
bool TreasurelandcoinNode_RespondToGetData(CNode *pnode);
bool TreasurelandcoinNode_SendInv(CNode *pnode);
bool TreasurelandcoinNode_AcceptData(CNode *pnode);
bool TreasurelandcoinNode_IgnoreIncoming(CNode *pnode);
bool TreasurelandcoinNode_IsLocal(CNode *pnode);
...
const string strMessageMagic = "TreasureLandCoin Signed Message:\n";
...
int TreasurelandcoinNode_ApplyUpgrades()
{
...
bool TreasurelandcoinNode_IsBlockChainSynced(CNode *pnode)
{
...
/*        
        if(mc_gState->m_Features->Streams())
        {
            if(!AcceptTreasureLandCoinTransaction(tx,view,-1,false,reason))
            {
                return state.DoS(0,
                                 error("AcceptToMemoryPool: : AcceptTreasureLandCoinTransaction failed %s : %s", hash.ToString(),reason),
...
/* MCHN START */
        ...
        if(mc_gState->m_Features->Streams())
        {
            if(!AcceptTreasureLandCoinTransaction(tx,view,-1,true,reason, &replay))
            {
                return state.DoS(0,
                                 error("AcceptToMemoryPool: : AcceptTreasureLandCoinTransaction failed %s : %s", hash.ToString(),reason),
                                 REJECT_NONSTANDARD, reason);
...
/* MCHN START */        
    if (block.GetHash() == Params().HashGenesisBlock()){        
        ...
        for (unsigned int i = 0; i < block.vtx.size(); i++)
        {
            ...
            if(mc_gState->m_Features->Streams())
            {
                if(!AcceptTreasureLandCoinTransaction(tx,view,offset,true,reason,NULL))
                {
...
/* MCHN START */        
            ...
            if(!fJustCheck)
            {
                if(mc_gState->m_Features->Streams())
                {
                    ...
                    {
                        return state.DoS(0,
                                         error("ConnectBlock: : AcceptTreasureLandCoinTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                         REJECT_NONSTANDARD, reason);
                    }
                }
                else
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (tx.IsCoinBase())
        {
            ...
            if(!fJustCheck)
            {
                if(mc_gState->m_Features->Streams())
                {
                    if(!AcceptTreasureLandCoinTransaction(tx,view,coinbase_offset,true,reason,NULL))
                    {
                        return false;       
                    }
                }
...
/* MCHN START */    
    ...
    TreasurelandcoinNode_ApplyUpgrades();        
    if(mc_gState->m_WalletMode & MC_WMD_TXS)
    {
...
/* MCHN START */    
    if(!TreasurelandcoinNode_RespondToGetData(pfrom)) 
    {
...
/* MCHN START */
        ...
        {            
            ...
            {
                PushTreasureLandCoinVerack(pfrom,false);
            }
        }
        else
        {
...
/* MCHN START */
        if(mc_gState->m_NetworkParams->IsProtocolTreasurelandcoin())
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(!ProcessTreasurelandcoinVerack(pfrom,vRecv,false,&disconnect_flag))
                    {
                    ...
                    else
                    {
                        ...
                        {
                            pfrom->fDisconnect = !PushTreasureLandCoinVerack(pfrom,true);
                        }
                        else
                        {
    ...
    else if (strCommand == "verackack")
    {
        if(mc_gState->m_NetworkParams->IsProtocolTreasurelandcoin())
        {
            ...
            if(!ProcessTreasurelandcoinVerack(pfrom,vRecv,true,&disconnect_flag))
            {
            ...
            {
                pfrom->fDisconnect |= !PushTreasureLandCoinVerack(pfrom,false);       // MCHN |= to avoid setting fDisconnect to false
            }
            if(pfrom->fDisconnect)
            {
                mc_gState->m_NetworkState = MC_NTS_SEED_READY;
...
/* MCHN END */
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    if(!TreasurelandcoinNode_IgnoreIncoming(pfrom))
                    {
        ...
        if (!vToFetch.empty() && !TreasurelandcoinNode_IgnoreIncoming(pfrom))      // MCHN
    ...
    else if (strCommand == "tx" && TreasurelandcoinNode_AcceptData(pfrom))            // MCHN
    {
/* MCHN START */        
        if(!TreasurelandcoinNode_IgnoreIncoming(pfrom))
        {
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            pfrom->PushMessage("getheaders", chainActive.GetLocator(pindexLast), uint256(0));
                                                                                ...
                                                                                // MCHN
    else if (strCommand == "block" && !fImporting && !fReindex && TreasurelandcoinNode_AcceptData(pfrom)) // Ignore blocks received while importing
    {
        CBlock block;
        vRecv >> block;
/* MCHN START */        
...
/* MCHN END */        
    ...
    else if (strCommand == "mempool" && TreasurelandcoinNode_SendInv(pfrom))          // MCHN
    {
...
/* MCHN START */    
    {        
        ...
        if(!pfrom->fDisconnect)
        {
            if(TreasurelandcoinNode_DisconnectRemote(pfrom))
            {            
                    pfrom->fDisconnect=true;
            ...
            if(!pfrom->fDisconnect)
            {
                if(pfrom->fCanConnectLocal && TreasurelandcoinNode_DisconnectLocal(pfrom))
                {
                        pfrom->fDisconnect=true;
...
/* MCHN END */            
        ...
        {
/* MCHN START */            
            if(pfrom->fDisconnect || !TreasurelandcoinNode_DisconnectRemote(pfrom))
            {
/* MCHN END */            
...
*/
                ...
                if (pto->setInventoryKnown.insert(inv).second)
                {
                    ...
                    if (vInv.size() >= 1000)
                    {
                        if(TreasurelandcoinNode_SendInv(pto))                         // MCNN
...
/* MCHN START */        
        ...
        {
            LOCK(cs_main);
            ignore_incoming=TreasurelandcoinNode_IgnoreIncoming(pto);
        }
        if(!ignore_incoming)
        {
...
/* MCHN START */        
        {
            LOCK(cs_main);
            CNode* seed_node;
            if(!pto->fSyncedOnce && TreasurelandcoinNode_IsBlockChainSynced(pto))
            {
                ...
                if(seed_node == pto)
                {
                    if(!TreasurelandcoinNode_IsLocal(pto))
                    {
