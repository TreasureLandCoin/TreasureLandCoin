...
bool ParseTreasurelandcoinTxOutToBuffer(uint256 hash,                                 // IN, tx hash, if !=0 genesis asset reference is retrieved from asset DB
...
*/
    ...
    else                                                                        // Protocol != treasurelandcoin
    {
        if(allowed)
        {
            *allowed=MC_PTP_SEND;
...
bool ParseTreasurelandcoinTxOutToBuffer(uint256 hash,                                 // IN, tx hash, if !=0 genesis asset reference is retrieved from asset DB
                                  ...
                                  string& strFailReason)                        // OUT error
{
    return ParseTreasurelandcoinTxOutToBuffer(hash,txout,amounts,lpScript,allowed,required,NULL,strFailReason);
}
