...
void mc_Params::Parse(int argc, const char* const argv[])
{
    ...
    if(m_NumArguments)
    {
        ...
        if((strcmp(exe_name,"treasurelandcoin-util") == 0) || (strcmp(exe_name,"treasurelandcoin-util.exe") == 0))
        {
            m_FirstArgumentType=MC_FAT_COMMAND;
        }
        else
        {
            if((strcmp(exe_name,"treasurelandcoin-cli") == 0) || 
               (strcmp(exe_name,"treasurelandcoin-cli.exe") == 0) ||                     
               (strcmp(exe_name,"treasurelandcoind") == 0) ||                     
               (strcmp(exe_name,"treasurelandcoind.exe") == 0))
            {
                ...
                if((m_FirstArgumentType == MC_FAT_NETWORK) && ((strcmp(exe_name,"treasurelandcoind") == 0) || (strcmp(exe_name,"treasurelandcoind.exe") == 0)))
                {
                    m_Arguments[m_NumArguments]=m_Arguments[0]+length;
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TreasureLandCoin";
#else
...
#else
    // Unix
    return pathRet / ".treasurelandcoin";
#endif
#endif
}
...
void mc_CheckDataDirInConfFile()
{
    ...
    mapConfig=new mc_MapStringString;
    if(mc_ReadGeneralConfigFile(mapConfig,NULL,"treasurelandcoin",".conf") == 0)
    {
...
{
    string fileName="treasurelandcoin";
    if(file_name)
    {
        fileName = file_name;
    }
...
{
    mc_ReadConfigFile(mc_GetConfigFile(network_name,"treasurelandcoin",".conf"),&mapArgs, &mapMultiArgs,"-");    
...
int mc_TreasurelandcoinParams::SetGlobals()
{
    m_IsProtocolTreasureLandCoin=1;
    void *ptr=GetParam("chainprotocol",NULL);
    if(ptr)
    {
        if(strcmp((char*)ptr,"treasurelandcoin"))
        {
            m_IsProtocolTreasureLandCoin=0;
        }
    }
...
int mc_TreasurelandcoinParams::Import(const char *name,const char *source_address)
{
    
    return MC_ERR_NOERROR;
}

std::string TreasurelandcoinServerAddress()
{
